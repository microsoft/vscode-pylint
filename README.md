...
-   Shipped `pylint` version is used by default. You can use `pylint` from your environment by setting `pylint.importStrategy` to `fromEnvironment`. You can use a custom `pylint` executable by setting `pylint.path`.
