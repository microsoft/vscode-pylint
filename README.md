...
- **Mono repo support**: If you are working with a mono repo, you can configure the extension to lint Python files in subfolders of the workspace root folder by setting the `pylint.cwd` setting to `${fileDirname}`.
...
### Disabling Pylint
If you wish to disable linting with Pylint, you can you can [disable this extension](https://code.visualstudio.com/docs/editor/extension-marketplace#_disable-an-extension) in Visual Studio Code. You can disable it for the current workspace or globally.
...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| pylint.args             | `[]`                                                                                                                                   | Arguments passed in to Pylint for linting Python files. Each argument should be provided as a separate string in the array. <br> Examples:  <br>- `"pylint.args": ["--rcfile=<file>"]` <br> - `"pylint.args": ["--disable=C0111", "--max-line-length=120"]`                                                                                                                                                                                                                                                                                                                                   |
...
| pylint.path             | `[]`                                                                                                                                   | Path or command to be used by the extension to lint Python files with Pylint. Accepts an array of a single or multiple strings. If passing a command, each argument should be provided as a separate string in the array. Note: Using this option may slowdown linting. <br>Examples: <br>- `["~/global_env/pylint"]` <br>- `["conda", "run", "-n", "lint_env", "python", "-m", "pylint"]`                                                                                                                                                                                  |
...
| pylint.importStrategy   | `useBundled`                                                                                                                           | Defines which Pylint to be used to lint Python files. When set to `useBundled`, the extension will use the Pylint binary that is shipped with the extension. When set to `fromEnvironment`, the extension will attempt to use the Pylint binary and all dependencies that are available in the currently selected environment. Note: If the extension can't find a valid Pylint binary in the selected environment, it will fallback to using the Pylint binary that is shipped with the extension The `pylint.path` setting may be ignored when this setting is set to `fromEnvironment`. |
...
The `pylint.path` setting also supports the `${interpreter}` variable, which is subtituted based on the value of the `pylint.interpreter` setting.
...
- If you are using a custom Pylint binary, make sure that the binary is compatible with the version of Python you have selected in your workspace. You can check the version of Python used by the extension by running the **Python: Select Interpreter** command from the Command Palette (**View** > **Command Palette...**).