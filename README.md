...
| pylint.cwd              | `${workspaceFolder}`                                                                                                                   | Sets the current working directory used to lint Python files with Pylint. By default, it uses the root directory of the workspace `${workspaceFolder}`. You can set it to `${fileDirname}` to use the parent folder of the file being linted as the working directory for Pylint. You can also set it to `${nearestConfig}` to use the nearest parent directory containing a pylint config file (`.pylintrc`, `pyproject.toml`, or `setup.cfg`) as the working directory. This is useful in monorepos.                                                                                                                              |
