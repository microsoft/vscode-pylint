...
def _setup_mocks():
    ...
    # lsprotocol mock — use __getattr__ to handle any unknown attribute
    class _AutoMod(types.ModuleType):
        def __getattr__(self, name):
            return type(name, (), {})()

    mock_lsprotocol = types.ModuleType("lsprotocol")
    mock_lsp = _AutoMod("lsprotocol.types")
    ...
    # Class-valued names — use auto-attribute class so nested access and
    # arbitrary instantiation both work (e.g. CodeActionOptions(...))
    ...
    def _make_auto(name):
        return _AutoClass(name, (_AnyInit,), {})
    ...
    for _name in [
        ...
        setattr(mock_lsp, _name, _make_auto(_name))
    ...
    mock_lsp_utils.is_current_interpreter = lambda p: False
    mock_lsp_utils.is_match = lambda patterns, path: False
    mock_lsp_utils.QuickFixRegistrationError = type(
