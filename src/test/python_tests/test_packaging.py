...
def test_pylint_metadata_version():
    """importlib.metadata must resolve the bundled pylint version."""
    version = importlib.metadata.version("pylint")
    assert version, "pylint version string should not be empty"
    parts = version.split(".")
